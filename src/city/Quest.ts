...
import { buildPerson, type CharRig } from './People';

export interface QuestStep {
  npc: string;
  x: number; z: number;
  target: { x: number; z: number };   // a dónde apunta la brújula/olfato
  clue: string;                        // lo que dice EN su turno (avanza la pista)
  filler: string;                      // lo que dice fuera de turno
}
...
const OWNER_LINE =
  '¡Canito! Perdoname, pulga… con el apuro se me soltó la correa y ni me di cuenta. ' +
...
const FALSE_SCENTS: Array<{ x: number; z: number; color: number }> = [
...
let _scentTex: THREE.Texture | null = null;
function scentTexture(): THREE.Texture {
  if (_scentTex) return _scentTex;
  const cv = document.createElement('canvas'); cv.width = cv.height = 64;
  const c = cv.getContext('2d')!;
  const g = c.createRadialGradient(32, 32, 1, 32, 32, 31);
  g.addColorStop(0, 'rgba(255,255,255,1)'); g.addColorStop(1, 'rgba(255,255,255,0)');
  c.fillStyle = g; c.beginPath(); c.arc(32, 32, 31, 0, Math.PI * 2); c.fill();
  _scentTex = new THREE.CanvasTexture(cv);
  return _scentTex;
}

function emojiTexture(char: string): THREE.CanvasTexture {
  const cv = document.createElement('canvas'); cv.width = cv.height = 64;
  const c = cv.getContext('2d')!;
...
const TRACES: Array<{ emoji: string; thought: string }> = [
...
const MEMORIES: Array<{ x: number; z: number; text: string }> = [
...
function resample(pts: Array<[number, number]>, n: number): Array<[number, number]> {
  ...
  if (total < 1e-3) { for (let i = 0; i < n; i++) out.push([pts[0][0], pts[0][1]]); return out; }
  for (let k = 0; k < n; k++) {
    let d = (k / (n - 1)) * total, i = 0;
    while (i < seg.length && d > seg[i]) { d -= seg[i]; i++; }
    if (i >= seg.length) { out.push([pts[pts.length - 1][0], pts[pts.length - 1][1]]); continue; }
...
function buildArrow(): THREE.Group {
  ...
  shaft.rotation.x = Math.PI / 2; shaft.position.z = -0.1; g.add(shaft);
  const head = new THREE.Mesh(new THREE.ConeGeometry(0.16, 0.34, 10), mat);
  head.rotation.x = Math.PI / 2; head.position.z = 0.3; g.add(head);
  return g;
}

export interface TalkResult { name: string; text: string; win?: boolean; }
...
export class QuestManager {
  ...
  private rigs: CharRig[] = [];
  private owner: CharRig;
  private star: THREE.Mesh;          // ✨ sobre el NPC activo
  private compass: THREE.Group;      // flecha-brújula sobre Canito
  private wisps: THREE.Sprite[] = [];        // rastro de olor (sobre el camino del dueño)
  private freshness = 0;                     // se desvanece con el tiempo al oler
  private legCached = -1;
  private falseScents: THREE.Sprite[] = [];  // olores falsos (sólo en modo olfato)
  private traceSprites: THREE.Sprite[] = []; // objetos que dejó el dueño
  private traceThought: boolean[] = STEPS.map(() => false);
  private memSprites: THREE.Sprite[] = [];   // recuerdos coleccionables
  private memDone: boolean[] = MEMORIES.map(() => false);
  memoriesFound = 0;
  readonly memoriesTotal = MEMORIES.length;
  private t = 0;
  ...
  constructor(scene: THREE.Scene) {
    ...
    for (let i = 0; i < WISPS; i++) {
      ...
      sp.scale.setScalar(1.7); sp.visible = false;
      scene.add(sp);
      this.wisps.push(sp);
    }
    ...
    STEPS.forEach((s, i) => {
      ...
      sp.position.set(s.x + 1.7, 0.9, s.z); sp.scale.setScalar(1.2); sp.visible = false;
      scene.add(sp);
    ...
    for (const m of MEMORIES) {
      ...
      sp.position.set(m.x, 1.0, m.z); sp.scale.setScalar(1.1);
      scene.add(sp);
      this.memSprites.push(sp);
    }
  }

  setSmell(on: boolean): void {
    this.smellMode = on;
    if (on) this.freshness = 1;     // al volver a oler, el rastro se "refresca"
  }

  /** Coloca las nubecitas de olor a lo largo del tramo `idx` que recorrió el dueño. */
  private _layLeg(idx: number): void {
    const path = resample(LEGS[idx], WISPS);
    for (let i = 0; i < WISPS; i++) this.wisps[i].position.set(path[i][0], 0.6, path[i][1]);
  }

  /** Si Canito está al lado del objeto del paso actual, devuelve su pensamiento
   *  (una sola vez por parada). */
  activeTraceThought(px: number, pz: number): string | null {
    const i = this.step;
    if (i >= STEPS.length || this.traceThought[i]) return null;
  ...
  collectMemory(px: number, pz: number): string | null {
    for (let i = 0; i < MEMORIES.length; i++) {
      if (this.memDone[i]) continue;
      const m = MEMORIES[i];
      if (Math.hypot(m.x - px, m.z - pz) < 2.4) {
        this.memDone[i] = true; this.memoriesFound++;
  ...
  update(dt: number, px: number, pz: number): { canTalk: boolean; who: string } {
    this.t += dt;
    ...
    if (leg !== this.legCached) { this._layLeg(leg); this.legCached = leg; }
    const smell = this.smellMode && !this.finished;
    if (smell) this.freshness = Math.max(0, this.freshness - dt * 0.13);  // se disipa (~7.5 s)
    if (this.smellMode && this.freshness <= 0) this.smellMode = false;     // se apaga solo
    ...
    for (let i = 0; i < this.traceSprites.length; i++) {
      ...
      sp.visible = show;
      if (show) sp.position.y = 0.95 + Math.sin(this.t * 3) * 0.12;
    }
  ...
  tryTalk(px: number, pz: number): TalkResult | null {
    ...
    for (const s of STEPS) {
      if (Math.hypot(s.x - px, s.z - pz) < TALK_R) return { name: s.npc, text: s.filler };
    }
    return null;
  }
}
