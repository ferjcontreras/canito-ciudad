...
export interface VehicleSound { x: number; z: number; speed: number; moto: boolean; }
...
export class CityAudio {
  ...
  private voices: {
    ...
    kind: 'car' | 'moto' | null;
    lp: BiquadFilterNode; gain: GainNode; pan: StereoPannerNode;
    cur: number;          // gain actual (para suavizar)
  }[] = [];

  // Samples reales
  private carEngine: AudioBuffer | null = null;
  private motoEngine: AudioBuffer | null = null;
  private carHorns: AudioBuffer[] = [];
  private footBuf: AudioBuffer | null = null;
  private barkBuf: AudioBuffer | null = null;

  // Pasos de gente (dos voces en loop, ganancia según gente cercana)
  private footGains: GainNode[] = [];
  private footStarted = false;
  private footCur = 0;

  // Tranvía (una voz en loop, ganancia/paneo según el tranvía más cercano)
  private tramEngine: AudioBuffer | null = null;
  private tramGain: GainNode | null = null;
  private tramPan: StereoPannerNode | null = null;
  private tramCur = 0;
  ...
  private px = 0; private pz = 0; private rx = 1; private rz = 0;

  // Estado para diagnóstico en pantalla.
  info = 'sin iniciar';
  get state(): string { return this.ctx ? this.ctx.state : 'no-ctx'; }
  get ready(): boolean { return this.carEngine != null; }
  activeVoices(): number { return this.voices.filter(v => v.cur > 0.01).length; }

  /** Prende/apaga la música. Devuelve el nuevo estado. */
  toggleMusic(): boolean {
    this.musicOn = !this.musicOn;
    if (this.musicOn) this._startMusic(); else this._stopMusic();
    return this.musicOn;
  }
  get musicPlaying(): boolean { return this.musicOn; }

  private _resume(): void {
    if (!this.ctx) return;
    this.ctx.resume().then(
      () => { this.info = 'resume() OK → ' + this.ctx!.state; },
      (e) => { this.info = 'resume() FALLÓ: ' + (e?.message ?? e); },
    );
  }
  ...
  unlock(): void {
    ...
    try {
      ...
      this.info = 'EXCEPCIÓN: ' + ((e as Error)?.message ?? e);
  ...
  private async _loadSamples(): Promise<void> {
    ...
    this.carEngine = ce;
    this.motoEngine = me;
    if (a) this.carHorns.push(a);
    if (b) this.carHorns.push(b);
    this.footBuf = ft;
    this.tramEngine = tr;
    this.musicBuf = mus;
    this.barkBuf = bk;
    if (this.musicOn) this._startMusic();   // ya con los buffers cargados
  }
  ...
  private _startMusic(): void {
    ...
    if (this.musicBuf) {                     // pista real en loop
      if (!this.musicSrc) {
        ...
        src.buffer = this.musicBuf; src.loop = true;
        const g = this.ctx.createGain(); g.gain.value = 0;
        src.connect(g).connect(this.master);
        src.start();
        this.musicSrc = src; this.musicGain = g;
      }
  ...
  private _stopMusic(): void {
    if (this.musicGain && this.ctx) this.musicGain.gain.setTargetAtTime(0, this.ctx.currentTime, 0.3);
    this.music?.stop();
  }
  ...
  private _setVoiceKind(voice: CityAudio['voices'][number], kind: 'car' | 'moto'): void {
    if (voice.kind === kind && voice.src) return;
    const buf = kind === 'moto' ? this.motoEngine : this.carEngine;
    if (!buf) return;                         // el sample todavía no cargó
    const ctx = this.ctx!;
    if (voice.src) { try { voice.src.stop(); } catch { /* ya parado */ } voice.src.disconnect(); }
    ...
    voice.src = src; voice.kind = kind;
  }

  /** Arranca las dos voces de pasos (una vez cargado el sample). */
  private _ensureFootsteps(): void {
    if (this.footStarted || !this.footBuf) return;
    this.footStarted = true;
    const ctx = this.ctx!;
    for (const [rate, pan] of [[0.98, -0.25], [1.07, 0.25]] as Array<[number, number]>) {
      const src = ctx.createBufferSource();
      src.buffer = this.footBuf; src.loop = true; src.playbackRate.value = rate;
  ...
  private _ensureTram(): void {
    if (this.tramGain || !this.tramEngine) return;
    const ctx = this.ctx!;
    const src = ctx.createBufferSource();
    src.buffer = this.tramEngine; src.loop = true;
    ...
    this.tramGain = g; this.tramPan = p;
  }
  ...
  update(
    ...
    walkLevel: number,        // 0..1 según cuánta gente camina cerca del jugador
    trams: { x: number; z: number }[] = [],
  ): void {
    if (!this.ctx || this.ctx.state !== 'running') return;
    this.acc += dt;
    if (this.acc < 0.08) return;      // ~12 Hz, evita thrash de parámetros
    const dt2 = this.acc; this.acc = 0;
    const ctx = this.ctx;
    ...
    const rx = fz, rz = -fx;
    this.px = px; this.pz = pz; this.rx = rx; this.rz = rz;

    // N vehículos más cercanos dentro del rango
    const near: { v: VehicleSound; d: number }[] = [];
    for (const v of vehicles) {
      const dx = v.x - px, dz = v.z - pz;
      const d = Math.hypot(dx, dz);
      if (d < ENGINE_RANGE) near.push({ v, d });
    }
    ...
    for (const g of this.footGains) g.gain.setTargetAtTime(this.footCur, now, 0.15);
  ...
  bark(): void {
    ...
    woof(now); woof(now + 0.2);   // "guau guau"
  }
  ...
  honkAt(x: number, z: number, moto: boolean): void {
    if (!this.ctx || this.ctx.state !== 'running') return;
    const ctx = this.ctx;
    const now = ctx.currentTime;
