...
interface Spot { x: number; z: number; reward: number; sp: THREE.Sprite; done: boolean; }
...
export class DigSpots {
  ...
  constructor(scene: THREE.Scene, positions: Array<[number, number]>) {
    ...
    for (const [x, z] of positions) {
      ...
      sp.position.set(x, 0.6, z); sp.scale.setScalar(1.2); sp.visible = false;
      scene.add(sp);
  ...
  update(dt: number, smell: boolean): void {
    this.t += dt;
    for (const s of this.spots) {
      ...
      if (show) {
        ...
        (s.sp.material as THREE.SpriteMaterial).opacity = 0.6 + (Math.sin(this.t * 4) + 1) * 0.2;
      }
    }
  }

  /** ¿Hay un tesoro acá? Si lo hay, lo cava y devuelve los huesos (0 si no). */
  digNear(px: number, pz: number): number {
    for (const s of this.spots) {
      if (s.done) continue;
      if (Math.hypot(s.x - px, s.z - pz) < 2.2) {
        s.done = true; s.sp.visible = false;
        return s.reward;
      }
    }
    return 0;
