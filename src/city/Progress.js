...
export class Progress {
    ...
    save() {
        try {
            localStorage.setItem(KEY, JSON.stringify({ huesos: this.huesos, repVecinos: this.repVecinos }));
        }
        catch { /* ignorar */ }
    }
    addHuesos(n) { this.huesos = Math.max(0, this.huesos + n); this.save(); }
    addRep(n) { this.repVecinos = Math.max(-100, Math.min(100, this.repVecinos + n)); this.save(); }
