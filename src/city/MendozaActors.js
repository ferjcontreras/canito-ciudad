...
export class MendozaActors {
    actors = [];
    jogGroup;
    jog = { cx: -1000, cz: -10, r: 28, a: 0, speed: 0.9 };
    constructor(scene) {
        for (const d of DEFS) {
    ...
    update(dt, px, pz) {
        ...
        let near = null;
        if (Math.hypot(jx - px, jz - pz) < 7)
            near = { id: 'a_corredor', text: '¡Uno corriendo! …yo mejor sigo buscando al humano.' };
        for (const a of this.actors) {
            const d = Math.hypot(a.x - px, a.z - pz);
            if (d < 6) {
                a.group.rotation.y = Math.atan2(px - a.x, pz - a.z); // mira a Canito
                if (!near)
                    near = { id: a.id, text: a.text };
            }
            else {
