...
async function main() {
    ...
    for (const way of streets) {
        ...
        for (const nid of way.nodes) {
            ...
            if (nd)
                p.push(proj.project(nd.lat, nd.lon));
        }
    ...
    const onRoad = (x, z, margin = 0.3) => {
        for (const s of roadSegs) {
            ...
            if (Math.hypot(x - px, z - pz) < s.hw + margin)
                return true;
        }
        return false;
    };
    ...
    for (let t = 20; t < avLen - 12; t += 18) {
        ...
        if (!onRoad(rx, rz))
            addAvenueTree(treeGeos, rx, rz, 0.9); // no plantar sobre las calles que cruza
        if (!onRoad(lx, lz))
            addAvenueTree(treeGeos, lx, lz, 0.9);
    }
    ...
    const audioEvents = ['keydown', 'pointerdown', 'click', 'touchstart'];
    const unlockAudio = () => {
        audio.unlock();
    ...
    const openTravel = () => {
        ...
        for (const d of TRAVEL) {
            ...
            b.onclick = () => {
                if (!progress.spend(TRAVEL_COST)) {
                    b.textContent = '¡No te alcanzan los huesos!';
                    return;
                }
                renderWallet();
    ...
    window.addEventListener('keydown', e => { if (e.code === 'KeyV')
    ...
    window.addEventListener('keydown', e => {
        if (e.code === 'KeyF')
            quest.setSmell(!quest.smellMode);
    });
    ...
    const win = () => {
        ...
        if (mem >= total) {
            endGame('🌟 ¡Final excelente!', `Encontraste a Don Ernesto Y todos los recuerdos de Canito (${mem}/${total}). Una vida entera juntos, ahora completa.`, '#ffe66a');
        }
        else if (dogs >= 3) {
            endGame('💚 ¡Buen final!', `Encontraste a Don Ernesto y, de paso, hiciste ${dogs} amigos perrunos en el camino. (Recuerdos: ${mem}/${total})`, '#7effd0');
        }
        else {
            endGame('🐾 ¡En casa!', `Canito reencontró a Don Ernesto en el lago del Parque. Vuelven juntos, al atardecer. (Recuerdos: ${mem}/${total})`, '#7eff8a');
        }
    };
    ...
    engine.start(dt => {
        ...
        if (over && performance.now() - overAt > 700 && input.keys.has('Space'))
            location.reload();
        ...
        pedBuf.length = 0;
        pedestrians.positions(pedBuf);
        for (const q of pedBuf)
            obstacles.push({ x: q.x, z: q.z, r: 0.5, honk: false });
        for (const t of trams)
            obstacles.push({ x: t.position.x, z: t.position.z, r: 14, honk: false });
        ...
        let pedsNear = 0;
        for (const q of pedBuf)
            if (Math.hypot(q.x - p.x, q.z - p.z) < 18)
                pedsNear++;
        ...
        if (!over) {
            ...
            engine.setDusk(nf);
            if (night >= 1)
                lose('🌙 Cayó la noche', 'Se hizo de noche y Canito no llegó. Se acurrucó solito en un banco de la plaza…');
            ...
            if (catHit.damage > 0) {
                ...
                if (catHit.attacked)
                    think('gato', '¡Un gato me arañó! ¡Fuera de acá, michi!');
            }
            // Ladrido (B): espanta a los gatos cercanos
            const barkNow = input.keys.has('KeyB');
            if (barkNow && !prevBark) {
                audio.bark();
                if (cats.scare(p.x, p.z, 10) > 0)
                    think('bark', '¡Guau guau! ¡A volar de acá, michis!');
            }
            prevBark = barkNow;
            ...
            if (trickNow && !prevTrick && trickCD <= 0) {
                ...
                let near = 0;
                for (const q of pedBuf)
                    if (Math.hypot(q.x - p.x, q.z - p.z) < 4.5)
                        near++;
                if (near > 0) {
                    const reward = Math.min(3, near);
                    progress.addHuesos(reward);
                    progress.addRep(2);
                    renderWallet();
                    for (let k = 0; k < reward; k++)
                        emitHeart(p.x + (Math.random() - 0.5) * 2, p.z + (Math.random() - 0.5) * 2);
            ...
            if (digNow && !prevDig) {
                ...
                if (got > 0) {
                    progress.addHuesos(got);
                    renderWallet();
                    think('dig', `¡Cavé y desenterré ${got} 🦴!`);
                }
            }
            prevDig = digNow;
            ...
            if (sustoCD <= 0) {
                ...
                for (const v of vehStates) {
                    if (v.speed > 1.2 && Math.hypot(v.x - p.x, v.z - p.z) < 3.0) {
                        hx = v.x;
                        hz = v.z;
                        hit = true;
                        break;
                ...
                if (!hit)
                    for (const t of trams) {
                        if (Math.hypot(t.position.x - p.x, t.position.z - p.z) < 4.5) {
                            hx = t.position.x;
                            hz = t.position.z;
                            hit = true;
                            break;
            ...
            for (let i = bones.length - 1; i >= 0; i--) {
                ...
                if (b.alive && Math.hypot(b.position.x - p.x, b.position.z - p.z) < Bone.PICKUP_R) {
                    b.pickup();
                    vida = Math.min(1, vida + 0.2);
                    b.remove(engine.scene);
                    bones.splice(i, 1);
                    progress.addHuesos(1);
                    renderWallet();
                    think('bone', '¡Un huesito! Lo guardo… y me hace bien.');
                }
            }
            if (vida <= 0)
                lose('💔 Sin vida', 'Canito quedó sin fuerzas. Cuidá la vida: esquivá los gatos y el tránsito, y comé huesitos.');
            // Pistas / diálogos
            const qs = quest.update(dt, p.x, p.z);
            if (qs.canTalk) {
                promptEl.textContent = `Apretá E para hablar con ${qs.who}`;
        ...
        if (performance.now() > thoughtHideAt)
        ...
        for (const h of hearts) {
            ...
            h.sp.material.opacity = Math.min(1, h.life);
            if (h.life <= 0)
                h.sp.visible = false;
        }
        if (hudLat && hudLon) {
