...
export class CatManager {
    cats = [];
    static DETECT = 11; // m: a partir de acá se ponen agresivos
    static ATTACK_R = 1.4;
    static SPEED = 3.6; // bastante menos que Canito (corre a 7.5)
    static DAMAGE = 0.12; // por arañazo
    ...
    _build(color) {
        ...
        for (const sz of [-0.07, 0.07]) {
            ...
            ear.position.set(0.34, 0.58, sz);
            g.add(ear);
            const eye = new THREE.Mesh(new THREE.SphereGeometry(0.03, 6, 4), new THREE.MeshBasicMaterial({ color: 0x9fe03a }));
            eye.position.set(0.47, 0.44, sz);
            g.add(eye);
        }
        for (const [lx, lz] of [[0.2, 0.09], [0.2, -0.09], [-0.2, 0.09], [-0.2, -0.09]]) {
    ...
    scare(px, pz, radius) {
        ...
        for (const c of this.cats) {
            if (Math.hypot(px - c.x, pz - c.z) < radius) {
                c.flee = 3.5;
                c.aggro = false;
                c.atkCD = 1;
                n++;
    ...
    update(dt, px, pz) {
        ...
        for (const c of this.cats) {
            ...
            if (c.flee > 0) {
                ...
                c.gait += dt * 13;
                c.tail.rotation.x = Math.sin(c.gait) * 0.2;
                continue;
            }
            ...
            if (c.aggro) {
                if (d > CatManager.ATTACK_R) {
                    ...
                    attacked = true;
                    c.x -= (dx / d) * 0.6;
                    c.z -= (dz / d) * 0.6; // retrocede tras el zarpazo
                }
