...
export class TrafficManager {
    vehicles = [];
    constructor(scene, graph, cars, motos) {
        // Carril derecho: centro del semicarril (~halfW/2 del eje).
        const laneOffset = (halfW) => halfW * 0.5;
        const spawn = (group, speed) => {
            const agent = new PathAgent(graph, laneOffset, speed);
            scene.add(group);
            this.vehicles.push({ agent, group });
        };
        for (let i = 0; i < cars; i++)
            spawn(buildCar(), 7 + Math.random() * 4); // ~25-40 km/h
        for (let i = 0; i < motos; i++)
            spawn(buildMoto(), 9 + Math.random() * 4);
    }
    update(dt) {
