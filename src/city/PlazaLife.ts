...
export class PlazaLife {
  private ws: Wanderer[] = [];
  private greeted = new Set<Wanderer>();   // perritos que llegaron a olfatear a Canito

  helpedDogs(): number { return this.greeted.size; }
  ...
  sniff(px: number, pz: number, radius: number): void {
    for (const w of this.ws) {
      if (w.kind !== 'dog') continue;
      const dx = px - w.x, dz = pz - w.z;
      const d = Math.hypot(dx, dz);
      if (d < radius) {
        if (d > 1.3) { w.tx = px; w.tz = pz; w.pause = 0; }   // va hacia él
        else { w.tx = w.x; w.tz = w.z; w.group.rotation.y = Math.atan2(dx, dz); this.greeted.add(w); }  // lo huele
      }
    }
  }
